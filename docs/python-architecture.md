...
| 场景                | 方式               | 说明                                                                                         |
| ------------------- | ------------------ | -------------------------------------------------------------------------------------------- |
| 企业微信 / 微信渠道 | **Tauri sidecar**  | PyInstaller 打包的 `channel-sidecar` 或开发态 `python -m channel`；双向 **stdio NDJSON RPC** |
...
- `wework` / `wechat` 登录、消息回调、发送、联系人同步触发
- 媒体下载与少量 SDK 必需格式转换
...
- `wework_channel + wework_message + run + ntwork glue`（及必要的 `wechat` 适配）
