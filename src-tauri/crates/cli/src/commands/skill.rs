...
use crate::skills_config::{
    self, hub_api_base, is_enabled_in_config, merge_disk_skills, register_skill, set_enabled,
};
...
fn list_local() -> Result<()> {
    let ws = paths::resolve_workspace()?;
    let mut config = skills_config::load(&ws);
    if merge_disk_skills(&ws, &mut config) {
        skills_config::save(&ws, &config)?;
    }
    if config.is_empty() {
...
fn list_remote(page: u32) -> Result<()> {
    ...
    let installed: std::collections::HashSet<_> = skills_config::load(&paths::resolve_workspace()?)
        .keys()
        .cloned()
        .collect();
...
fn print_skill_table<'a>(entries: Vec<&'a skills_config::SkillConfigEntry>, title: &str) {
    println!("\n  {title} ({})\n", entries.len());
    for e in entries {
