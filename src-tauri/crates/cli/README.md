...
```bash
# from repo root
cargo build --manifest-path src-tauri/Cargo.toml -p cli --release
cargo run --manifest-path src-tauri/Cargo.toml -p cli -- --help
...
```bash
cargo install --path src-tauri/crates/cli --locked
```
