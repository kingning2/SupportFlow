...
def _login_qr_png_bytes(login_url: str) -> io.BytesIO:
    """Build WeChat login QR as PNG (uses project dependency `qrcode`)."""
    ...
    qr.make(fit=True)
    qr.make_image(fill_color="black", back_color="white").save(qrStorage, format="PNG")
    qrStorage.seek(0)
    return qrStorage


def _login_qr_ascii_text(login_url: str) -> str:
    import qrcode as qr_lib
    ...
    qr.make(fit=True)
    return "\n".join(
        "".join("1" if cell else "0" for cell in row) for row in qr.modules
    )
...
def get_QR(self, uuid=None, enableCmdQR=False, picDir=None, qrCallback=None):
    ...
    login_url = 'https://login.weixin.qq.com/l/' + uuid
    qrStorage = _login_qr_png_bytes(login_url)
    if hasattr(qrCallback, '__call__'):
        ...
        if enableCmdQR:
            utils.print_cmd_qr(_login_qr_ascii_text(login_url), enableCmdQR=enableCmdQR)
        else:
