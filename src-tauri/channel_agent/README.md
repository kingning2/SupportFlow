...
Tauri 启动时会注入 `CHANNEL_CONFIG_PATH` / `TAURI_CHANNEL_MODE=1`。

**wework** 要求 **Python 3.10**（3.12 无 ntwork wheel）。`ntwork` 不在 PyPI，由 `setup:channel-sidecar-dev` / 打包脚本从 [ntwork-bin-backup](https://github.com/hanfangyuan4396/ntwork-bin-backup) 安装。
...
| 变量 | 说明 |
|------|------|
| `CHANNEL_SIDECAR_PYTHON` | 打包用解释器（建议 3.10） |
| `CHANNEL_BOOTSTRAP_PYTHON` | 下载 wheel 用的解释器（默认 `python`，常为 3.12） |
| `CHANNEL_NTWORK_WHEEL` | ntwork `.whl` 的绝对路径 |
| `CHANNEL_SKIP_WEWORK_DEPS` | `1` 时跳过 wework 依赖检查 |

产物：`src-tauri/binaries/channel-sidecar-<target>.exe`（PyInstaller **单文件**，内含 Python 3.10 运行时 + ntwork/pilk 等，不依赖用户机上的 Python）。
