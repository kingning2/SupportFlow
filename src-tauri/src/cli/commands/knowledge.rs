...
use crate::services::agent::knowledge::KnowledgeService;
...
fn stats() -> Result<String> {
    ...
    fn count_nodes(
        nodes: &[crate::services::agent::knowledge::KnowledgeTreeNode],
...
fn upload(paths: &[PathBuf], category: &str) -> Result<()> {
    ...
    for src in paths {
        ...
        let data = crate::io::read(src).with_context(|| format!("read {}", src.display()))?;
