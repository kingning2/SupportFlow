...
use crate::config::{Context as AgentContext, ContextType};
use anyhow::Result;
...
async fn chat_once(
    ...
    let on_event = if verbose {
        Some(
            std::sync::Arc::new(move |ev: crate::services::agent::AgentEvent| {
                eprintln!("[agent] {ev:?}");
            }) as crate::services::agent::AgentEventCallback,
        )
    } else {
        None
    };
...
fn format_reply(reply: &crate::config::Reply) -> String {
    reply
        .text_content
        .clone()
        .filter(|s| !s.is_empty())
