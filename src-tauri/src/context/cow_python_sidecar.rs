    ...
    async fn stdout_reader_loop(
        ...
        loop {
            ...
            let n = match read_line_lossy(reader, &mut line).await {
                Ok(n) => n,
                Err(e) => {
                    crate::log_warn!("[cowagent] stdout reader ended: {e}");
                    break;
                }
            };
            if n == 0 {
            ...
            let msg: Value = match serde_json::from_str(trimmed) {
                Ok(v) => v,
                Err(e) => {
                    crate::log_debug!("[cowagent] non-json stdout: {trimmed} ({e})");
                    continue;
                }
            };
...
pub fn resolve_python_executable() -> String {
    if let Some(exe) = cow_python_from_env() {
        return exe;
    }
    if cfg!(windows) {
        for launcher in [("py", ["-3.8"]), ("py", ["-3.10"])] {
            if let Some(exe) = python_executable_from_launcher(launcher.0, &launcher.1) {
                return exe;
            }
        }
        "python".to_string()
...
fn cow_python_from_env() -> Option<String> {
    if let Ok(exe) = std::env::var("COW_PYTHON_EXECUTABLE") {
        let trimmed = exe.trim();
        if !trimmed.is_empty() {
            return Some(trimmed.to_string());
        }
    }
    option_env!("COW_PYTHON_EXECUTABLE").and_then(|exe| {
        let trimmed = exe.trim();
        if trimmed.is_empty() {
            None
        } else {
...
const EMBEDDED_CHANNEL_SIDECAR: &[u8] = include_bytes!(concat!(
    ...
    "/binaries/cowagent-channels-",
    env!("BUILD_TARGET"),
    ".exe"
));

/// Release: sidecar PyInstaller exe is linked into `tauri-app` at compile time (`include_bytes`).
/// First run extracts it to app cache (no separate cowagent-channels exe beside the installer).
#[cfg(channel_sidecar_embedded)]
fn materialize_embedded_sidecar(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    let exe_name = format!("cowagent-channels{}", std::env::consts::EXE_SUFFIX);
...
fn resolve_sidecar_paths(app: &AppHandle) -> (PathBuf, Option<PathBuf>) {
    // Dev: prefer `python -m channel` so stale PyInstaller binaries do not pollute stdout.
    #[cfg(debug_assertions)]
    if std::env::var("COW_CHANNEL_SIDECAR_EXE").is_err() {
        if let Some(dev) = dev_source_dir() {
            return (PathBuf::new(), Some(dev));
        }
    }

    if let Ok(raw) = std::env::var("COW_CHANNEL_SIDECAR_EXE") {
...
fn dev_source_dir() -> Option<PathBuf> {
    let src = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("cowagent");
...
pub async fn spawn_sidecar(
    app: &AppHandle,
    config_path: &Path,
) -> Result<Arc<CowPythonSidecar>, String> {
    ...
    let sidecar = CowPythonSidecar::new(sidecar_exe, config_path.to_path_buf(), dev);
