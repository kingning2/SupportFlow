# Download cp38 Windows wheels with a working pip (default: python 3.12).
# ntwork is not on PyPI; we fetch the cp38 wheel from ntwork-bin-backup on GitHub.
$ErrorActionPreference = "Stop"
$Root = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path
$WheelDir = Join-Path $Root "src-tauri\cowagent\_wheels\sidecar-pip"
$BootstrapPy = if ($env:COW_BOOTSTRAP_PYTHON) { $env:COW_BOOTSTRAP_PYTHON } else { (Get-Command python -ErrorAction Stop).Source }
...
$sidecarReq = Join-Path $Root "src-tauri\cowagent\requirements-sidecar.txt"
